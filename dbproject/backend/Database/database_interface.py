...
class Database:
    def __init__(self, host='localhost', database='TEST', user='root', password='caramel'):
        try:
    ...
    def createUser(self, name, email, hashed_password):
        insert_statement = "INSERT INTO users (name, email, hashed_password) VALUES (%s, %s, %s)"
        insert_tuple = (name, email, hashed_password)
    ...
    def create_admin(self, name, email, password):
        ...
        sql = "insert into admin_users values('"+str(
            name)+"', '"+str(email)+"', '"+str(password)+"', NULL)"

        cursor.execute(sql)
        self.__connection.commit()

    def validate_admin(self, name, email, password):
        cursor = self.__connection.cursor()
