...
class Database:
    ...
    def createApplication(self, name, user_id, job_id, degree, university, degree_level, a_levels, languages, employment, skills, hobbies, response, feedback, cover_letter, scores):
        # Take in strings, JSON strings and list of floats
        print(degree_level)
        print(university)
        print(degree)
        # a_levels = json.dumps(a_levels)
        insert_statement = "INSERT INTO userjobs (name, user_id, job_id, degree, university, degree_level, a_levels, languages, employment, skills, hobbies, response, net_feedback, cover_letter, score_degree, score_alevels, score_lang, score_employment, score_skills, userjob_id) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"
        print(degree_level)
        insert_tuple = (name, user_id, job_id, degree, university, degree_level, a_levels, languages, employment, skills,
    ...
    def get_new_interviews_json(self):
        ...
        for row in rows:
            ...
            sql = "select firstname, lastname from users where id = "+str(user_id)
            cursor.execute(sql)
            ...
            a_levels = json.loads(row[6])

            languages = json.loads(row[7])
            employment = json.loads(row[8])
            skills = json.loads(row[9])
            hobbies = json.loads(row[10])
            response = row[11]
            score_degree = row[14]
            score_alevels = row[15]
            score_lang = row[16]
            score_employment = row[17]
            score_skills = row[18]
            id = row[19]
            ...
            print(university)
            print(degree)
            data_send = {"Name": name, "DegreeQualification": json.loads(degree), "DegreeLevel": json.loads(degree_level), "UniversityAttended": json.loads(university), "ALevelQualifications": a_levels, "LanguagesKnown": languages, "PreviousEmployment": employment,
                         "Skills": skills, "Hobbies": hobbies, "Response": response, "score_degree": score_degree, "score_alevels": score_alevels, "score_lang": score_lang, "score_employment": score_employment, "score_skills": score_skills, "ID": id,
    ...
    def get_applications(self, user_id):
        ...
        for i in row:
            ...
            data_send = {"Name": name, "DegreeQualification": degree, "DegreeLevel": degree_level, "UniversityAttended": university, "ALevelQualifications": json.loads(a_levels),
                         "LanguagesKnown": json.loads(languages), "PreviousEmployment": json.loads(employment), "Skills": json.loads(skills), "Hobbies": json.loads(hobbies), "ID": application_id, "Feedback": feedback, "Response": response}
    ...
    def get_jobs_available(self):
        ...
        for row in cursor.fetchall():
            ...
            data_send = {"ID": id, "Title": title, "Description": description, "Responsibilities": responsibilities, "PeopleManagement": people_management,
                         "SkillsNeeded": skills_needed, "IdealCandidate": ideal_candidate, "Education": education, "Location": location}
    ...
    def get_jobs(self):
        ...
        for row in cursor.fetchall():
            ...
            data_send = {"ID": id, "Title": title, "Description": description, "Responsibilities": responsibilities, "PeopleManagement": people_management,
                         "SkillsNeeded": skills_needed, "IdealCandidate": ideal_candidate, "Education": education, "Location": location, "DatePosted": date_posted, "DateExpiry": date_expiry}
            jobs.append(data_send)
    ...
    def validate_admin(self, email, password):
        cursor = self.__connection.cursor()
        sql = "select email, hashed_password from admin_users where email = '" + \
            str(email)+"'"
        cursor.execute(sql)
        res = cursor.fetchone()
        if res == None:
    ...
    def get_token(self, email, admin):
        ...
        sql = "select token from tokens where user_id = {0} and is_admin = {1}".format(
            id, admin)
        cursor.execute(sql)
    ...
    def does_token_exist(self, token):
        cursor = self.__connection.cursor()
        sql = "select * from tokens where token = '"+str(token) + "'"
        cursor.execute(sql)
        res = cursor.fetchone()
        return not (res == None)
    ...
    def is_admin(self, token):
        cursor = self.__connection.cursor()
        sql = "select is_admin from tokens where token = '{0}'".format(str(token))
        cursor.execute(sql)
        return cursor.fetchone()[0]
    ...
    def get_feedback_by_user_id(self, user_id):
        cursor = self.__connection.cursor()
        sql = "select * from userjobs where user_id = {0}".format(user_id)
        cursor.execute(sql)
        ...
        for i in applications:
            ...
            data_send = {"Name": name, "DegreeQualification": degree, "DegreeLevel": degree_level, "UniversityAttended": university, "ALevelQualifications": a_levels, "LanguagesKnown": languages, "PreviousEmployment": employment,
                         "Skills": skills, "Hobbies": hobbies, "Response": response, "ID": id, "user_id": user_id, "job_id": job_id}

            
            formatted_applications.append(data_send)

        print()
        return json.dumps(formatted_applications)
